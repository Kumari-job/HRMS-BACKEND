...
class EmployeeLeaveController extends Controller
{
    ...
    public function store(EmployeeLeaveRequest $request)
    {
        try {
            ...
            if ($employee_leave) {
                ...
                $employee_leave_statuses = collect($request->requested_to)
                    ->map(function ($requested_to) use ($employee_leave) {
                        return [
    ...
    public function update(EmployeeLeaveRequest $request, $id)
    {
        ...
        if (
            EmployeeLeaveStatus::where('employee_leave_id', $employee_leave->id)
                ->whereNotNull('status')
                ->exists()
        ) {
